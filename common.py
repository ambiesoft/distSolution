...
class DistConfig:
    ...
    def __init__(self, path):
        with open(path, encoding="utf-8-sig") as data_file:
    ...
    def checkAlreadyUploaded(self, verstring):
        ...
        if IsRemoteArchiveExists(url):
            myexit(
                "Archive already exists in remote site {0}. quitting.".format(url))
...
def createGitRev(gitrev, ShowDummy=False, DummyType='cpp', Char='char'):
    ...
    if ShowDummy:
        ...
        for gitdir in gitrev['gitdirs']:
            dir = os.path.basename(os.path.abspath(gitdir)).replace(
                '.', '').replace('/', '').replace('\\', '')
            if not dir:
                exit('dir is empty')
    ...
    if (gitrev and ('outheader' in gitrev or 'outcsharp' in gitrev)) or (ShowDummy and (DummyType == 'cpp' or DummyType == 'csharp')):
        if ShowDummy:
            ...
            if 'outheader' in gitrev and 'outcsharp' in gitrev:
                exit(
                    'Both "outheader" and "outcsharp" are specified. Only one of them is allowed.')
            if 'outheader' in gitrev:
                outfile = gitrev['outheader']
                # check extension
                if lspy.getExtension(outfile) != '.h' and lspy.getExtension(outfile) != '.hpp':
                    exit(
                        "'outheader specified but its extesnion is wrong. it must be '.h' or '.hpp'")
            elif 'outcsharp' in gitrev:
                outfile = gitrev['outcsharp']
                # check extension
                if lspy.getExtension(outfile) != '.cs':
                    exit(
                        "'outheader specified but its extesnion is wrong. it must be '.cs'")
                DummyType = 'csharp'
            if not outfile:
                exit('No outfile')
